...
module AppReputation
  class IosRatingsFetcherForItunesClient
    ...
    def get_ratings(id, *countries)
      ...
      case countries.size
      when 0
        get_multiple_ratings(id, *@@supported_countries.list)
      when 1
        get_single_ratings(id, countries.first)
      else
        get_multiple_ratings(id, *countries)
      end
    end
    
    def get_single_ratings(id, country)
    ...
    def get_multiple_ratings(id, *countries)
      jobs = countries.inject([]) do |memo, country|
        job = Proc.new do
          get_single_ratings(id, country)
        end
        memo.push(job)
      end
    ...
    private :get_single_ratings, :get_multiple_ratings
  end
end
